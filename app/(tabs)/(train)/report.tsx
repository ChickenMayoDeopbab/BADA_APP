...
export default function Report() {
  const { sessionId, mode = "scenario" } = useLocalSearchParams<ReportParams>();
  const [feedback, setFeedback] = useState<TrainingFeedbackResponse | null>(null);
  ...
  const fetchFeedback = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setError(null);
      setFeedback(await getTrainingFeedbackBySessionId(sessionId));
    } catch (requestError) {
      console.error("[Report] 피드백 조회 실패", requestError);
          ...
          <View className="items-center pt-12">
            ...
            {mode === "scenario" && (
              <Text className="mb-1 text-base font-medium text-[#3B3D3E]">
                시나리오명 <Text className="font-bold">{feedback.scenarioName}</Text>
              </Text>
            )}
